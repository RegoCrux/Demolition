    ...
    public class AccountController : Controller
    {
        ...
        public ActionResult LogOn(LogOnModel model, string returnUrl)
        {
            if (ModelState.IsValid)
            {
                ...
                if (userModel != null)
                {
                    HttpCookie UNHC = new HttpCookie("username", userModel.Name);
                    Response.Cookies.Add(UNHC);
                    HttpCookie PWHC = new HttpCookie("password", Models.User.EncryptPassword(userModel.Password,"tUst6-u3anuyu8u-"));
        ...
        public ActionResult Register(RegisterModel model)
        {
            if (ModelState.IsValid)
            {
                ...
                if (user != null)
                {
                    ...
                    HttpCookie UNHC = new HttpCookie("username", user.Name);
                    Request.Cookies.Add(UNHC);
                    HttpCookie PWHC = new HttpCookie("password", Models.User.EncryptPassword(user.Password, "tUst6-u3anuyu8u-"));
